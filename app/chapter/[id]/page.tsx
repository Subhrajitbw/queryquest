        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'learn' && (
            <motion.div 
              ...
              className="flex-1"
            >
              <h2 className="text-2xl font-bold text-white mb-6">{chapterData.learn.title}</h2>
              <div className="prose prose-invert max-w-none">
                {chapterData.learn.content.split('\n\n').map((paragraph: string, i: number) => (
                  <p key={i} className="text-gray-300 text-lg leading-relaxed mb-6">{paragraph}</p>
                ))}
              </div>
                ...
                <div className="mt-8">
                  ...
                  <div className="space-y-4">
                    {chapterData.learn.examples.map((example: string, i: number) => (
                      <div key={i} className="bg-[#0a0f1c] border border-white/10 rounded-lg p-4 font-mono text-sm text-blue-300">
                        {example}
                      </div>
                    ))}
                  </div>
          ...
          {activeTab === 'practice' && (
            ...
            >
              ...
              {practiceResult && (
                <div className="mb-6">
                  <div className={`p-4 rounded-lg mb-4 flex items-center gap-3 ${isPracticeCorrect ? 'bg-green-500/10 border border-green-500/30 text-green-400' : 'bg-yellow-500/10 border border-yellow-500/30 text-yellow-400'}`}>
